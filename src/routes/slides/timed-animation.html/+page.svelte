<!--
  ...
  File: src/routes/slides/timed-animation.html/+page.svelte
...
<script>
	...
	const path = 'src/routes/slides/timed-animation.html/+page.svelte';

	// --- How you'd AUTHOR this slide's position stops with the live helper ----------
	// KeyframeStudio drives positions only (left/top/width/height); the opacity + scale
...
<!--
  SKETCH — authoring the positions above with KeyframeStudio (commented out; this
  ...
  <KeyframeStudio name="appear-move" initialStops={stops} duration={5}>
      🎬&nbsp; New Option
  </KeyframeStudio>
