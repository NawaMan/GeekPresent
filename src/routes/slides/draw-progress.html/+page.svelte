<!--
  ...
  File: src/routes/slides/draw-progress.html/+page.svelte
  ...
  % and a drawn % field per keyframe — the reveal is editable on all three —
  plus a LIVE readout (▸ 48% · 62% drawn) that tracks the playhead as you
  drag the ANIMATE bar. The panel stays up while you scrub.
-->
<script lang="ts">
	...
	const path = 'src/routes/slides/draw-progress.html/+page.svelte';
</script>
...
<ContentPage title="Draw — Keyframed Progress" subtitle="Keyframe the DRAW progress on every shape — Line, Curve, Arc">
	<div style="line-height: 1.4em;">
		<p>
			...
			go. Flip <b>LAYOUT</b> on, select any shape, and edit each keyframe's
			<b>%</b> and <b>drawn %</b> right in the panel — which shows a
			<b>live ▸ progress · drawn</b> readout that follows the playhead as you
			press <b>ANIMATE</b> and drag the bar (the panel stays up while you scrub).
		</p>
		<QuickCode style="margin-top: 0.4em;">
			&lt;Line stops=&#123;[&#123; pct: 0, drawn: 0 &#125;, &#123; pct: 75, drawn: 0.5 &#125;, &#123; pct: 100, drawn: 1 &#125;]&#125; animate=&#123;4&#125; /&gt;
		</QuickCode>
	</div>
</ContentPage>
...
<Draw title="Keyframed draw-progress on a line, a curve and an arc" name="progress"
	...
	<Line name="beam" from={[360, 550]} to={[1640, 550]} stops={[{ pct: 0, drawn: 0 }, { pct: 75, drawn: 0.95 }, { pct: 100, drawn: 1 }, { pct: 25, drawn: 0.05 }]} animate={4} color="#f39c12" thickness={7} />

	<!-- Curve: geometry (c1 pulls up → more bend) + non-linear reveal. -->
	<Curve name="sweep" from={[360, 780]} to={[1640, 720]} c1={[1000, 900]} stops={[{ pct: 0, c1: [1000, 900], drawn: 0 }, { pct: 75, drawn: 0.95 }, { pct: 100, c1: [1001, 520], drawn: 1 }, { pct: 25, c1: [1000, 900], drawn: 0.05 }]} animate={4} color="#4aa3ff" thickness={7} />

	<!-- Arc: bend morph (opens up) + non-linear reveal. -->
	<Arc name="loop" from={[360, 980]} to={[1640, 980]} bend={0.12} stops={[{ pct: 0, bend: 0.12, drawn: 0 }, { pct: 75, drawn: 0.95 }, { pct: 100, bend: 0.34, drawn: 1 }, { pct: 25, bend: 0.12, drawn: 0.05 }]} animate={4} color="#00c56a" thickness={7} />
</Draw>
